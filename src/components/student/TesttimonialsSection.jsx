...
const TesttimonialsSection = () => {
  ...
  return (
    <div className='pb-14 px-8 md:px-0'>
      ...
      <div className='grid md:grid-cols-3 repeat(auto-fit, minmax(200px, 1fr)) gap-8 mt-14 m-14'>
        {dummyTestimonial.map((testimonial, index) => (
          <div key={index} className='bg-white rounded-lg  text-sm text-left border
           border-gray-500/30 pb-6 shadow-[0px_4px_15px_0px] shadow-black/5 overflow-hidden '>
            ...
            <div className='p-5 pb-7'>
                <div className='flex gap-0.5'>
                  {[...Array(5)].map((_, i) =>(<img  key={i}  src={i < Math.floor(testimonial.rating) ? assets.star : assets.star_blank} alt=''
                             className='h-5'  />
                  ))}        
                </div>
